...
function AppShell({ session, onLogout, darkMode, onToggleTheme }) {
  ...
  const toStateLabel = (value) => {
    ...
    if (['true', 'enable', 'enabled', 'on', 'yes'].includes(normalized)) return 'Enabled'
    if (['false', 'disable', 'disabled', 'off', 'no'].includes(normalized)) return 'Disabled'
    return String(value)
  }
                    ...
                    {wafPolicies.length === 0 ? (
                      ...
                      <div className="waf-card-grid">
                        {filteredWafPolicies.map((policy, index) => {
                          ...
                          const certCn = clientCertificateDetails.subject || clientCertificateDetails.cn || '-'
                          const certIssuer = clientCertificateDetails.issuer || '-'
                          const certExpireDate = toDateOnly(clientCertificateDetails.valid_to || clientCertificateDetails.expire_date)
                          const certDaysLeft = clientCertificateDetails.days_left ?? '-'
                          const tlsV10 = typeof policy === 'string' ? null : policy.tls_v10
                          const tlsV11 = typeof policy === 'string' ? null : policy.tls_v11
                          const tlsLegacyState = tlsV10 === null && tlsV11 === null ? '-' : (tlsV10 || tlsV11)
                          ...
                          const policyCardId = `${policyName}-${index}`
                          const isExpanded = expandedPolicyCard === policyCardId
                          return (
                          <article
                            className={`policy-card ${isExpanded ? 'selected' : ''}`}
                            key={`${policy._deviceName}-${policyName}-${index}`}
                            onClick={() => setExpandedPolicyCard((prev) => (prev === policyCardId ? '' : policyCardId))}
                          >
                            <div className="policy-top-row">
                              ...
                              <div className="policy-status-wrap">
                                ...
                                <span className={`policy-expand-icon ${isExpanded ? 'expanded' : ''}`} aria-hidden="true">
                            ...
                            {isExpanded && (
                              <div className="policy-summary-wrap">
                                ...
                                <div className="policy-summary-grid">
                                  <section className="policy-summary-card">
                                    <p className="policy-summary-title">Endpoint</p>
                                    <p className="policy-summary-row"><span>IP</span><strong>{policyIp || '-'}</strong></p>
                                    <p className="policy-summary-row"><span>SNI</span><strong>{toStateLabel(sni)}</strong></p>
                                    <p className="policy-summary-row"><span>Hostname</span><strong>{hostname}</strong></p>
                                    <p className="policy-summary-row"><span>Traffic Mirror</span><strong>{toStateLabel(trafficMirror)}</strong></p>
                                  </section>
                                  <section className="policy-summary-card">
                                    <p className="policy-summary-title">Certificate</p>
                                    <p className="policy-summary-row"><span>CN</span><strong>{certCn}</strong></p>
                                    <p className="policy-summary-row"><span>Issuer</span><strong>{certIssuer}</strong></p>
                                    <p className="policy-summary-row"><span>Expire Date</span><strong>{certExpireDate}</strong></p>
                                    <p className="policy-summary-row"><span>Days Left</span><strong>{certDaysLeft}</strong></p>
                                  </section>
                                  <section className="policy-summary-card">
                                    <p className="policy-summary-title">Network</p>
                                    <p className="policy-summary-row"><span>TLS v1.3</span><strong>{toStateLabel(tlsV13)}</strong></p>
                                    <p className="policy-summary-row"><span>TLS v1.2</span><strong>{toStateLabel(tlsV12)}</strong></p>
                                    <p className="policy-summary-row"><span>TLS v1.0-1.1</span><strong>{toStateLabel(tlsLegacyState)}</strong></p>
                                    <p className="policy-summary-row"><span>HTTP/2</span><strong>{toStateLabel(http2)}</strong></p>
                                  </section>
                                </div>
                              </div>
                            )}
